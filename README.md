...
- 256 GB Drive at `/dev/sda`
- 16 GB RAM

I won't know the specific partition names (e.g. `/dev/sda1` being `boot`), so
...
Partition with `cgdisk` or `fdisk`.

Format partitions:
```
$ # mkfs.vfat -c -F 32 -n BOOT /dev/boot-partition # can be formatted when partitioning
